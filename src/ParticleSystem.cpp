...
void GravityForce::setGravity(const ofVec3f &gravity)
{
...
void TurbulenceForce::setRange(const ofVec3f &tmin, const ofVec3f &tmax) {
	this->tmin = tmin;
	this->tmax = tmax;
}
...
void ImpulseRadialForce::updateForce(Particle * particle) {
	...
	//
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(-height/2, height/2), ofRandom(-1, 1));
	particle->forces += dir.getNormalized() * magnitude;
}

void ImpulseRadialForce::setHeight(float theight)
{
	this->height = theight;
}

void ImpulseRadialForce::setMagnitude(float tmagnitude)
{
	this->magnitude = tmagnitude;
}
